...
export function JoinForm({ email }: { email: string }) {
  ...
  const field =
    'mt-2 w-full rounded-xl border border-rule-strong bg-page px-4 py-3 text-ink focus:border-accent focus:outline-none';
  const label = 'block text-xs font-semibold uppercase tracking-[0.12em] text-ink-faint';
  ...
  return (
    <form onSubmit={handleSubmit} className="measure">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          <label htmlFor="join-village" className={label}>
            Village
          </label>
          <input id="join-village" name="village" type="text" required className={field} />
        </div>
        <div className="sm:col-span-2">
          ...
          <p className="mt-1.5 text-xs text-ink-faint">
            Not required — but most of our work starts with somebody knowing somebody, so it helps.
          </p>
        </div>
      </div>
      ...
      <button
        type="submit"
        className="group mt-7 inline-flex items-center gap-2 rounded-full bg-accent px-7 py-3.5 font-semibold text-page transition-colors hover:bg-accent-strong"
      >
        Send enquiry
